...
7. For security reason, please generate new application key.
	...
	```
8.  Create your own plugin
	```
	php artisan create:plugin Foo.Bar
	```
9.  In your Foo.Bar `Plugin.php` file, add code bellow to configure lighthouse configruration
```php
...
```
10. Publish your lighthouse configuration with command `php artisan vendor:publish` and register the lighthouse configuration in `config/lighthouse` with following code. You can find `schema.graphql` in `Plugins/Foo/Barr/Graphql/Schema.graphql`
```php
    'schema' => [
...
11. Open url `{{your_domain}}/graphql-playground` you can find schema configuration on this panel.
...
* [Lighthouse Developer Guide](https://lighthouse-php.com/4.16/getting-started/installation.html#install-via-composer)
